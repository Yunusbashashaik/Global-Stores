...
export default function CartPopup({ open, onClose, lang, t }) {
  ...
  return (
    <div className="cart-popup" role="dialog" aria-label={t.cartTitle}>
      <header className="cart-popup-header">
        <h3>{t.cartTitle}</h3>
        <button type="button" className="cart-popup-close" onClick={onClose} aria-label={t.close}>
          ×
        </button>
      </header>
            ...
            return (
              <li key={item.key} className="cart-item">
                ...
                <div className="cart-item-meta">
                  <strong>{name}</strong>
                  <span>
                    {durationLabel(item.duration, t)} · {item.unitPrice} {currency}
                  </span>
      ...
      <footer className="cart-popup-footer">
        ...
        {items.length ? (
          <button type="button" className="btn btn-ghost cart-clear" onClick={clearCart}>
            {t.cartClear}
          </button>
        ) : null}
      </footer>
