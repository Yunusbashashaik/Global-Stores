...
- **Static hosting (GitHub Pages, GoDaddy static/HTML, etc.):** the form emails via **FormSubmit**. The first submission sends an **Activate Form** link to that inbox — click it once, then later complaints arrive by email with:
  ...
  - **Body** = Full Name, Phone Number, Subject, Complaint Details
  - **Screenshot** = a dedicated **Screenshot / Screenshot URL** row (hosted image link). FormSubmit often drops binary attachments, so the image is delivered as an openable link in the email body (attachment is still attempted).
